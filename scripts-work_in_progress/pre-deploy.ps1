...
process {
  ...
  $ROOT_DIR = "C:\Code\CT\subscriptions\sub_ipam"
  ...
  try {
    ...
    #### Terraform Deployment Here ####
    #### Use values from $appDetails and inject into tfvars.
    ...
    $TfappServiceName = "prod-az-app334526919" #$appServiceName = $deployment.appServiceName
    $TfappServiceHostName = "$TfappServiceName.azurewebsites.net" #$appServiceHostName = "${$deployment.appServiceName}.azurewebsites.net"
    $TfresourceGroupName = "rg-ipam-prd" #resourceGroupName = $deployment.resourceGroupName

    if (($PSCmdlet.ParameterSetName -notin 'AppsOnly') -and (-not $DisableUI)) {
      Update-UIApplication `
        -UIAppId $appDetails.UIAppId `
        -Endpoint $TfappServiceHostName
    }
    ...
    if ($PSCmdlet.ParameterSetName -in ('App', 'Function')) {
      ...
      try {
        Publish-ZipFile -AppName $TfappServiceName -ResourceGroupName $TfresourceGroupName
      } catch {
        Write-Host "SWITCH: Retrying ZIP Deploy with Kudu API..." -ForegroundColor Blue
        Publish-ZipFile -AppName $TfappServiceName -ResourceGroupName $TfresourceGroupName -UseAPI
      }
    }
    ...
    if ($PSCmdlet.ParameterSetName -in ('AppContainer', 'FunctionContainer') -and $PrivateAcr) {
      ...
      if($Function) {
        ...
        Restart-AzFunctionApp -Name $TfappServiceName -ResourceGroupName $TfresourceGroupName -Force | Out-Null
      } else {
        ...
        Restart-AzWebApp -Name $TfappServiceName -ResourceGroupName $TfresourceGroupName | Out-Null
      }
    }
    ...
    if ($($PSCmdlet.ParameterSetName -notin 'AppsOnly') -and (-not $DisableUI) -and $ParameterFile) {
      ...
      $updateAddr = "https://$($TfappServiceHostName)"
    ...
    if ($script:deploymentSuccess) {
      Write-Output "ipamURL=https://$($TfappServiceHostName)" >> $Env:GITHUB_OUTPUT
      ...
      Write-Output "ipamSuffix=$($Tfsuffix)" >> $Env:GITHUB_OUTPUT
      Write-Output "ipamResourceGroup=$($TfresourceGroupName)" >> $Env:GITHUB_OUTPUT
    }
